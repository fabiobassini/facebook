...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        window = UIWindow(frame: myWindow.coordinateSpace.bounds)
        window?.windowScene = myWindow
        window?.makeKeyAndVisible()


        currentUser = UserDefaults.standard.object(forKey: "currentUser") as? NSDictionary
//        print(currentUser as Any)

        //checking if the glob variable that stores current user's info is empty or not
        if currentUser?["id"] != nil {
            //accessing TabBar Controller via Storyboard
